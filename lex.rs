			...
			tokstrm.push_back(match c {
							...
							_ 			=>
								if is_op_char(c) {
									...
									src.make_token(|s| Punc(s.next().unwrap().1))
								} else {
									src.get_id()
								}
						})
...
impl CharsPos<'_> {
	...
	{
		let begin = self.pos;
		self.read_while(cond)
							.parse::<T>()
							.unwrap_or_else(|_| panic!(":{}:{} malformed {} literal", begin.row, begin.col, std::any::type_name::<T>()))
	}
	...
	fn get_strlit(&mut self) -> Token {
		...
		self.skip_char('"');
		let strlit = self.make_token(|s| Literal(StrLit(un_escape(s.read_lit(|&ch|{
															let cont = ch != '"' || escaped;
															escaped = !escaped && ch == '\\';
															cont
														})))));
	...
	fn get_numlit(&mut self) -> Token {
		...
		let num_lit = Literal(if is_flt {
			FltLit(to_num_lit
							.parse::<f64>()
							.unwrap_or_else(|_| panic!(":{}:{} malformed floating-point literal `{}`", startpos.row, startpos.col, to_num_lit)))
		} else {
			IntLit(to_num_lit
							.parse::<i64>()
							.unwrap_or_else(|_| panic!(":{}:{} malformed integer literal", startpos.row, startpos.col)))
		});

		Token { start: startpos, val: num_lit, end: self.pos }
	}
	...
	fn skip_comment(&mut self) {
		...
		while comment_depth != 0 {
			match self.next() {
				...
				Some((_, c))	=>	comment_depth += match c {
										'«'		=> 1,
										'»' 	=> -1,
										_		=> 0
									}
			...
			F: FnMut(&mut CharsPos) -> TokenValue
	{
		...
		let value = f(self);
		let endpos = self.pos;
		Token { start: startpos, val: value, end: endpos }
	}
}
...
fn map_op(opstr: String, prev_op: Option<&TokenValue>) -> TokenValue {
	match opstr.as_str() {
		"!"		=> UnaryOp(Not),
		":"		=> Punc(':'),
		"-" if !matches!(prev_op, Some(Literal(_))) &&
				!matches!(prev_op, Some(Ident(_)))
				=> UnaryOp(Minus),
		":="	=> AssignOp(Eq),
		op		=> BinaryOp(match_dict(BINARY_OP_DICT.iter().cloned(), op)
