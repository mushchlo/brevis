...
impl Inference {
	...
	fn occurs_in(&mut self, index: u16, t: Type) -> bool {
		match t {
			...
			Struct(s) => s.into_iter().any(|a| self.occurs_in(index, a.r#type)),
		...
		match a {
			AST::LetNode(l) => {
				...
				if let Some(def) = l.def {
					...
					self.constraints.push(Constraint::Equal((*def).r#type, def_t.clone()));
					self.constraints.push(Constraint::Equal(l.var.r#type, def_t));
				}
				Void
			},
			AST::ExprNode(expr) => {
				let expr_type = match expr.val.clone() {
					ExprVal::LiteralNode(lit) =>
						match lit {
							AtomicLiteral(StrLit(_)) => Type::Str,
							AtomicLiteral(IntLit(_)) => Type::Int,
							AtomicLiteral(FltLit(_)) => Type::Float,
							AtomicLiteral(BoolLit(_)) => Type::Bool,
							StructLiteral(s) =>
								Type::Struct(
									s.iter().map(|a|
										AggregateType {
											name: a.name.clone(),
											r#type: self.infer_type(AST::ExprNode(a.val.clone())),
										}
									).collect()
								),
						},
					...
					ExprVal::BinaryNode(b) if b.op == OpID::Member => {
						let t_left = self.infer_type(AST::ExprNode(*b.left.clone()));

						self.constraints.push(Constraint::Equal(t_left.clone(), b.left.r#type.clone()));

						let mut op_constraints = b.associations();
						self.constraints.append(&mut op_constraints);

						if let Struct(s) = t_left.clone() {
							if let Some(i) = s.iter().position(
									|a| matches!(b.right.val.clone(), ExprVal::IdentNode(s) if s == a.name)) {
								self.constraints.push(Constraint::Equal(b.right.r#type.clone(), s[i].r#type.clone()));
							}
						}


						b.op.result(t_left, b.right.r#type)
					}

					ExprVal::BinaryNode(b) => {
						let (t_left, t_right) = (
							self.infer_type(AST::ExprNode(*b.left.clone())),
							self.infer_type(AST::ExprNode(*b.right.clone()))
						);

						self.constraints.push(Constraint::Equal(t_left.clone(), b.left.r#type.clone()));
						self.constraints.push(Constraint::Equal(t_right.clone(), b.right.r#type.clone()));

						let mut op_constraints = b.associations();
						self.constraints.append(&mut op_constraints);

						b.op.result(t_left, t_right)
					}
					...
					ExprVal::IfNode(i) => {
						...
						if let Some(expr) = i.r#else {
								...
								self.constraints.push(Constraint::Equal(if_t.clone(), else_t));
						}
						self.constraints.push(Constraint::Equal(if_t.clone(), i.then.r#type.clone()));
						self.constraints.push(Constraint::Equal(cond_t.clone(), i.cond.r#type.clone()));
						self.constraints.push(Constraint::Equal(cond_t, Type::Bool));
					...
					ExprVal::LambdaNode(l) => {
						...
						self.constraints.push(Constraint::Equal(return_type.clone(), body_type));
					...
					ExprVal::CallNode(c) => {
						...
						self.constraints.push(Constraint::Equal(t1.clone(), c.func.r#type.clone()));
						...
						self.constraints.push(Constraint::Equal(t1, TypeConstructor(
							TConstructor {
								name: "Function".to_string(),
								args: arg_types
							}
				...
				self.constraints.push(Constraint::Equal(expr_type.clone(), expr.r#type));
				expr_type
			}
		}
	}
	...
	fn solve_constraints(&mut self) {
		...
		// TODO: there should be some unifyMembers that collects the members for each struct type and verifies them
		...
		match t {
			...
			Struct(s) => Struct(
				s.iter().map(|a|
					AggregateType {
						name: a.name.clone(),
						r#type: self.substitute(a.r#type.clone()),
					}
				).collect()
			),
		}
...
impl Expr {
	fn annotate_helper(&mut self, inference: &mut Inference) {
		...
		match self.val {
			ExprVal::LiteralNode(AtomicLiteral(_)) | ExprVal::IdentNode(_) => {},
			ExprVal::LiteralNode(StructLiteral(ref mut s)) => {
				for member in s.iter_mut() {
					member.val.annotate_helper(inference);
				}
			}
			...
			ExprVal::LambdaNode(ref mut l) => {
				for var in &mut l.args {
					var.r#type = inference.substitute(var.r#type.clone());
				}
